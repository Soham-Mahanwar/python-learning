...
### Packaging for Google AI Studio

1. Ensure secrets are not committed. Copy `.env.example` -> `.env` locally and fill values.
2. Create the sanitized zip (Windows):
...
3. Upload `project-ai-studio.zip` to Google AI Studio's notebook or project import.

Notes:
- The package script excludes `.git`, `venv`, `.env` and common caches.
- Ensure `requirements-ai-studio.txt` contains any extra packages your code needs.
